...
int is_space(char c) {
  if (c == 32 || c == 10) {return 1;} else {return 0;}
}
int is_linebreak(char c) {
  if (c == 10) {return 1;} else {return 0;}
}
int ascii_to_digit(char c) {
  return c - 48;
}
int is_start_comment(char c) {
  if (c == 37) {return 1;} else {return 0;}
}
...
int parse_one(int prev_ch, struct Token *out_token) {
  ...
  if (is_num(prev_ch)) {
    ...
    return peek;
  } else if (is_start_comment(prev_ch)) {
    while (!is_linebreak(peek = cl_getc())) {}
    out_token->ltype = SPACE;
    return peek;
  }
